...
async function getQuotations(): Promise<Quotation[]> {
  const query = `
    *[_type == "quotation" && salesPerson == "Aziz Ahmed"] | order(date asc, _createdAt asc) {
      quotationId,
      referenceNo,
      date,
      salesPerson,
          ...
          <div className="space-y-4">
            {filteredQuotations.length > 0 ? (
              ...
              <div className="text-center py-10 text-gray-500">
                {searchTerm ? 'No matching quotations found for Anas Nayyar' : 'No quotations found for Anas Nayyar'}
              </div>
            )}
          </div>
        ) : (
                  ...
                  filteredQuotations.map((q) => {
                    ...
                    return (
                      <tr key={q.quotationId} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-sm font-medium">
                          <Link
  href={`/Aziz-Ahmed/Quotation/${encodeURIComponent(q.quotationId?.toUpperCase() || "")}`}
  ...
  View
                  ...
                  <tr>
                    <td colSpan={6} className="px-4 py-6 text-center text-sm text-gray-500">
                      {searchTerm ? 'No matching quotations found for Aziz Ahmed' : 'No quotations found for Aziz Ahmed'}
                    </td>
                  </tr>
                )}
              </tbody>
