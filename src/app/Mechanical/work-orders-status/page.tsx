...
export default function WorkOrderDashboard() {
      ...
      try {
        ...
        const query = `
          *[_type == "workOrderSalesOrder"] {
            _id,
            _createdAt,
            "workOrderNumber": workOrderSection.workOrderNumber,
            "clientName": workOrderSection.clientName,
            "jobReference": workOrderSection.jobReference,
            "poNumber": salesOrderSection.orderDetails.poNumber,
            "deliveryDate": workOrderSection.deliveryDate,
            "status": workOrderSection.status.currentStatus,
            "items": workOrderSection.products[] {
        ...
        `
        const data: WorkOrder[] = await client.fetch(query)
        setWorkOrders(data)
      } catch (err) {
  ...
  const handleStatusChange = async (id: string, newStatus: WorkOrderStatus) => {
    try {
      ...
      const response = await fetch('/api/work-order-status/update', {
        method: 'POST',
          ...
          <div className="overflow-x-auto pb-2 -mx-4 px-4">
            <div className="flex md:grid md:grid-cols-3 lg:grid-cols-6 gap-2 md:gap-4 min-w-max md:min-w-0">
              {Object.entries(statusCounts).map(([status, count]) => (
                <button
                  key={status}
                  onClick={() => setStatusFilter(status === 'All' ? 'All' : status as WorkOrderStatus)}
                        ...
                        <tr key={workOrder._id} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium flex gap-2 justify-end">
                            ...
                            <select
                              ...
                              onChange={(e) => handleStatusChange(workOrder._id, e.target.value as WorkOrderStatus)}
