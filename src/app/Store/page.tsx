...
//   // Calculate square feet from MM: (Width MM × Length MM) / 92903
//   const calculateSqftFromMM = (widthMM: number | '', lengthMM: number | '') => {
//     if (widthMM === '' || lengthMM === '' || !widthMM || !lengthMM) return 0
//     const sqftValue = (widthMM * lengthMM) / 92903
//     return Number(sqftValue.toFixed(2))
//   }

//   // Calculate weight: SQFT × Gauge Thickness (mm)
//   const calculateWeight = (widthMM: number | '', lengthMM: number | '', gauge: string): number => {
//     if (widthMM === '' || lengthMM === '' || !widthMM || !lengthMM || !gauge) return 0
    
//     const thickness = GAUGE_TO_THICKNESS[gauge] || 0
//     const sqft = calculateSqftFromMM(widthMM, lengthMM)
    
//     if (thickness === 0 || sqft === 0) return 0
    
//     const weight = sqft * thickness
//     return Number(weight.toFixed(2))
//   }

//   // Calculate sheet cost based on weight and price per kg
//   const calculateSheetCost = (weight: number | '', material: string, gauge: string) => {
//     if (weight === '' || !weight || weight === 0) return 0
//     if (!gauge || !material) return 0
    
//     const priceEntry = livePrices?.sheetPrices?.find(
//       p => p.material === material && p.gauge === gauge
//     )
//     const pricePerKg = priceEntry?.pricePerKg || 0
//     return Number((pricePerKg * weight).toFixed(2))
//   }

//   // Calculate paint cost - MULTIPLY BY 2 FOR BOTH SIDES
//   const calculatePaintCost = (sqft: number | '') => {
//     if (sqft === '' || !sqft) return 0
//     const pricePerSqft = livePrices?.paintPrice || 0
//     return Number((pricePerSqft * sqft * 2).toFixed(2))
//   }

//   // Parse bulk entry text - QTY and MOQ always 10
//   const parseBulkEntry = (text: string) => {
...
//       // Format: Part #, Part Name, Category, Location, Mat/Gauge, W(mm), L(mm), SQFT, Wt(kg), Stock, Unit, Sheet, Paint
//       // Example: FPL 12M W4	Safety Plate 12M H600	finished	0	MS/22G	622	260	1.74	1.32	5	Pieces	363.34	285.4814161
//       // QTY will always be 10, MOQ will always be 10
      
//       if (parts.length >= 10) {
//         let partNumber = parts[0].trim()
//         let partName = parts[1].trim()
//         let category = parts[2].trim().toLowerCase()
//         let storeLocation = parts[3].trim()
        
//         // Handle material/gauge (e.g., "MS/22G" or "GI/20G")
//         let material = 'GI'
//         let gauge = '20G'
//         const materialGauge = parts[4].trim()
//         if (materialGauge.includes('/')) {
//           const [mat, g] = materialGauge.split('/')
//           material = mat.trim()
//           gauge = g.trim()
//         } else {
//           // Try to parse as gauge only
//           gauge = materialGauge
//         }
        
//         let width = parseFloat(parts[5].trim())
//         let length = parseFloat(parts[6].trim())
        
//         // SQFT is at index 7, Weight at index 8, Stock at index 9
//         let sqft = parseFloat(parts[7].trim()) || 0
//         let weight = parseFloat(parts[8].trim()) || 0
        
//         // Check if there's a Unit field (index 10)
//         let unitOfMeasure = 'Pieces'
//         if (parts.length > 10) {
//           unitOfMeasure = parts[10].trim()
//         }
        
//         // Check if there are Sheet Cost and Paint Cost fields
//         let sheetCost = 0
//         let paintCost = 0
//         if (parts.length > 11) {
//           sheetCost = parseFloat(parts[11].trim()) || 0
//         }
//         if (parts.length > 12) {
//           paintCost = parseFloat(parts[12].trim()) || 0
//         }
        
//         // If weight is 0, calculate it
//         if (weight === 0) {
//           weight = calculateWeight(width, length, gauge)
//         }
        
//         // If sqft is 0, calculate it
//         if (sqft === 0) {
//           sqft = calculateSqftFromMM(width, length)
//         }
        
//         // If costs are 0, calculate them
//         if (sheetCost === 0) {
//           sheetCost = calculateSheetCost(weight, material, gauge)
//         }
//         if (paintCost === 0) {
//           paintCost = calculatePaintCost(sqft)
//         }
...
//           storeLocation: storeLocation || 'Default',
//           material,
//           gauge,
//           blankWidthMM: width,
//           blankLengthMM: length,
//           sqft,
//           weight,
//           sheetCostPerPiece: sheetCost,
//           paintCostPerPiece: paintCost,
//           stockInStore: 10, // Always 10
//           minimumStockLevel: 10, // Always 10
//           unitOfMeasure
//         })
//       }
//     }
...
//   // Handle bulk entry change
//   const handleBulkEntryChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
//     const text = e.target.value
//     setBulkEntryText(text)
...
//       toast.error('No valid items to add. Please check the format.')
//       return
//     }
...
//         // Get current prices
//         const priceEntry = livePrices?.sheetPrices?.find(
//           p => p.material === item.material && p.gauge === item.gauge
//         )
//         const currentPricePerKg = priceEntry?.pricePerKg || 0
...
//           storeLocation: item.storeLocation || 'Default',
//           gauge: item.gauge || '20G',
//           material: item.material || 'GI',
...
//         const response = await fetch('/api/store/add-item', { 
//           method: 'POST', 
...
//         if (!response.ok) {
//           console.error('API Error for', item.partNumber, result)
//           errorCount++
//         } else {
//           successCount++
...
//         }
//       } catch (error) {
//         console.error('Error adding item:', item.partNumber, error)
//         errorCount++
//       }
//     }
...
//     if (successCount > 0) {
//       toast.success(`Successfully added ${successCount} items! ${errorCount > 0 ? `Failed to add ${errorCount} items.` : ''}`, { duration: 5000 })
//     } else {
//       toast.error(`Failed to add all ${bulkPreview.length} items. Please check the format and try again.`)
//     }
...
//   // Handle mm input and auto-calculate everything
...
//       const widthMM = name === 'blankWidthMM' ? mmValue : updated.blankWidthMM
//       const lengthMM = name === 'blankLengthMM' ? mmValue : updated.blankLengthMM
      
//       const newSqft = calculateSqftFromMM(widthMM, lengthMM)
//       updated.sqft = newSqft
      
//       let newWeight = 0
//       if (updated.gauge && widthMM && lengthMM) {
//         newWeight = calculateWeight(widthMM, lengthMM, updated.gauge)
//         updated.weight = newWeight
//       }
      
//       updated.sheetCostPerPiece = calculateSheetCost(newWeight, updated.material, updated.gauge)
...
//       let newWeight = prev.weight !== '' ? Number(prev.weight) : 0
      
//       if (prev.blankWidthMM && prev.blankLengthMM && prev.gauge) {
//         newWeight = calculateWeight(prev.blankWidthMM, prev.blankLengthMM, prev.gauge)
//         updated.weight = newWeight
//       }
      
//       updated.sheetCostPerPiece = calculateSheetCost(newWeight, value, prev.gauge)
...
//       let newWeight = 0
//       if (prev.blankWidthMM && prev.blankLengthMM) {
//         newWeight = calculateWeight(prev.blankWidthMM, prev.blankLengthMM, value)
//         updated.weight = newWeight
//       }
      
//       updated.sheetCostPerPiece = calculateSheetCost(newWeight, prev.material, value)
...
//     e.preventDefault()
//     if (!formData.partNumber || !formData.partName || !formData.storeLocation) { 
//       toast.error('Please fill in all required fields'); 
//       return 
//     }
//     if (formData.stockInHand === '' || formData.minOrderQty === '') { 
//       toast.error('Please enter stock information'); 
//       return 
//     }
...
//       let weightValue = formData.weight === '' ? 0 : Number(formData.weight)
      
//       if (weightValue === 0 && formData.blankWidthMM && formData.blankLengthMM && formData.gauge) {
//         weightValue = calculateWeight(
...
//       const calculatedPaintCost = currentPaintPrice * sqftValue * 2
...
//         throw new Error(result.error || result.details || 'Failed to add item')
//       }
      
//       setStoreItems(prev => [result.data, ...prev])
//       toast.success(`Item added successfully! Weight: ${weightValue.toFixed(2)} kg, Sheet Cost: Rs ${calculatedSheetCost.toFixed(2)}, Paint Cost: Rs ${calculatedPaintCost.toFixed(2)}`, { duration: 4000 })
...
//                     <td class="numeric-cell">${(item.weight || 0).toFixed(2)}</td>
...
//                   <td class="numeric-cell">Rs ${returnTotalCost.toFixed(2)}</td>
//                   </tr></tbody>
...
//         {/* Bulk Entry Section */}
//         <div className="bg-gradient-to-r from-purple-50 to-pink-50 rounded-lg p-6 border-2 border-purple-300 shadow-lg">
//           <h2 className={`text-xl font-bold text-purple-800 mb-2 ${dmSans.className}`}>📋 Bulk Entry</h2>
//           <p className="text-sm text-gray-600 mb-4">
//             Paste multiple items at once. Each line should follow this format (tab-separated):<br />
//             <span className="font-mono text-xs bg-white p-1 rounded border">Part #	Part Name	Category	Location	Mat/Gauge	W(mm)	L(mm)	SQFT	Wt(kg)	Stock	Unit	Sheet	Paint</span>
//             <br />
//             <span className="font-mono text-xs text-gray-500">Example: FPL 12M W4	Safety Plate 12M H600	finished	0	MS/22G	622	260	1.74	1.32	5	Pieces	363.34	285.48</span>
//             <br />
//             <span className="font-bold text-green-600 text-xs">Note: Quantity (QTY) and Min Order Qty (MOQ) will always be set to 10 automatically.</span>
//           </p>
//           <form onSubmit={handleBulkSubmit}>
//             <div className="space-y-4">
//               <div>
//                 <label className="block text-sm font-medium text-gray-700 mb-2">Paste your data here *</label>
//                 <textarea
//                   value={bulkEntryText}
//                   onChange={handleBulkEntryChange}
//                   rows={8}
//                   className="w-full px-4 py-3 border rounded-md focus:ring-2 focus:ring-purple-500 outline-none font-mono text-sm"
//                   placeholder="FPL 12M W4	Safety Plate 12M H600	finished	0	MS/22G	622	260	1.74	1.32	5	Pieces	363.34	285.48&#10;FPL 8M W8	Safety Plate 8M H400	finished	0	MS/20G	400	600	2.58	1.87	8	Pieces	412.50	325.75"
//                 />
//               </div>
              
//               {bulkPreview.length > 0 && (
//                 <div className="bg-white rounded-lg p-4 border border-purple-200">
//                   <h3 className={`text-sm font-semibold text-gray-700 mb-3 ${dmSans.className}`}>Preview ({bulkPreview.length} items found)</h3>
...
//                           <th className="px-2 py-1 text-left">Mat/Gauge</th>
//                           <th className="px-2 py-1 text-right">W(mm)</th>
//                           <th className="px-2 py-1 text-right">L(mm)</th>
//                           <th className="px-2 py-1 text-right">SQFT</th>
//                           <th className="px-2 py-1 text-right">Wt(kg)</th>
//                           <th className="px-2 py-1 text-right">QTY</th>
//                           <th className="px-2 py-1 text-left">Unit</th>
//                           <th className="px-2 py-1 text-right">Sheet</th>
//                           <th className="px-2 py-1 text-right">Paint</th>
...
//                             <td className="px-2 py-1 font-medium">{item.partNumber}</td>
//                             <td className="px-2 py-1">{item.partName}</td>
//                             <td className="px-2 py-1 capitalize">{item.category}</td>
//                             <td className="px-2 py-1">{item.storeLocation}</td>
//                             <td className="px-2 py-1">{item.material}/{item.gauge}</td>
//                             <td className="px-2 py-1 text-right">{item.blankWidthMM}</td>
//                             <td className="px-2 py-1 text-right">{item.blankLengthMM}</td>
//                             <td className="px-2 py-1 text-right">{item.sqft.toFixed(2)}</td>
//                             <td className="px-2 py-1 text-right">{item.weight.toFixed(2)}</td>
//                             <td className="px-2 py-1 text-right font-bold text-blue-600">10</td>
//                             <td className="px-2 py-1">{item.unitOfMeasure}</td>
//                             <td className="px-2 py-1 text-right">Rs {item.sheetCostPerPiece.toFixed(2)}</td>
//                             <td className="px-2 py-1 text-right">Rs {item.paintCostPerPiece.toFixed(2)}</td>
//                             <td className="px-2 py-1 text-right font-bold text-green-600">10</td>
//                           </tr>
//                         ))}
//                       </tbody>
//                     </table>
...
//                       <p className="text-gray-500">Total items: <span className="font-bold">{bulkPreview.length}</span></p>
//                       <p className="text-gray-500">QTY per item: <span className="font-bold text-blue-600">10</span></p>
//                       <p className="text-gray-500">MOQ per item: <span className="font-bold text-green-600">10</span></p>
//                     </div>
//                     <button
//                       type="submit"
//                       disabled={isBulkSubmitting || bulkPreview.length === 0}
//                       className="px-6 py-2 bg-purple-600 text-white rounded-md hover:bg-purple-700 disabled:opacity-50 font-medium"
//                     >
//                       {isBulkSubmitting ? `Adding ${bulkPreview.length} items...` : `Add All ${bulkPreview.length} Items →`}
//                     </button>
//                   </div>
//                 </div>
//               )}
...
//               <div className="mb-4">
//                 <p className="text-xs text-gray-500">Paint Price (per sqft) - Both Sides</p>
//                 <p className="text-lg font-bold text-blue-700">Rs {livePrices?.paintPrice?.toFixed(2) || '0.00'} PKR</p>
//                 <p className="text-xs text-gray-400">Updated: {formatDate(livePrices?.paintEffectiveDate)}</p>
//               </div>
//               <div>
//                 <p className="text-xs text-gray-500 mb-2">Material Prices (per kg)</p>
//                 <div className="overflow-x-auto">
//                   <table className="min-w-full text-sm">
//                     <thead>
//                       <tr className="text-xs text-gray-600">
//                         <th className="text-left py-1">Material</th>
//                         <th className="text-left py-1">Gauge</th>
//                         <th className="text-right py-1">Price per Kg (PKR)</th>
//                       </tr>
//                     </thead>
//                     <tbody>
...
//                         <tr><td colSpan={3} className="text-center text-gray-400 py-2">No material prices configured</td></tr>
//                       )}
//                     </tbody>
//                   </table>
//                 </div>
...
//                       <td className="px-2 py-2 whitespace-nowrap text-[11px] text-right text-gray-700">{(item.sqft || 0).toFixed(2)}</td>
...
//                       <td className="px-2 py-2 whitespace-nowrap text-[11px] text-right text-gray-700">Rs {sheetCost.toFixed(2)}</td>
//                       <td className="px-2 py-2 whitespace-nowrap text-[11px] text-right text-gray-700">Rs {paintCost.toFixed(2)}</td>
...
//                   <p className="font-medium">{(returnItem.weight || 0).toFixed(2)} kg</p>
...
//                   <h3 className={`text-lg font-semibold text-blue-800 mb-4 ${dmSans.className}`}>🎨 Paint Price (per sqft) - Both Sides</h3>
//                   <div>
...
//                     </div>
//                     <p className="text-xs text-gray-500 mt-1">Current price: Rs {livePrices?.paintPrice?.toFixed(2) || '0.00'} PKR per sqft (will be multiplied by 2 for both sides)</p>
//                   </div>
//                 </div>
...
//                     <p>Weight = SQFT × Gauge Thickness (mm)</p>
//                     <p>SQFT = (Width mm × Length mm) / 92903</p>
//                     <p>Paint cost is calculated as: SQFT × Paint price per sqft × 2 (both sides)</p>
//                   </div>
...
//                               <option value="10G">10G (2.313mm)</option>
//                               <option value="12G">12G (1.834mm)</option>
//                               <option value="14G">14G (1.454mm)</option>
//                               <option value="16G">16G (1.159mm)</option>
//                               <option value="18G">18G (0.918mm)</option>
//                               <option value="20G">20G (0.725mm)</option>
...
//                       <p className="text-2xl font-bold text-blue-600">{(formData.sqft || 0).toFixed(2)} sqft</p>
//                       <p className="text-xs text-gray-500">(Width mm × Length mm) ÷ 92903</p>
//                     </div>
//                     <div className="bg-white rounded-lg p-4 shadow-sm">
//                       <label className="block text-sm font-medium text-gray-700 mb-2">Weight (kg) - AUTO</label>
//                       <p className="text-2xl font-bold text-teal-600">{(formData.weight || 0).toFixed(2)} kg</p>
//                       <p className="text-xs text-gray-500">SQFT × Gauge Thickness (mm)</p>
//                     </div>
//                     <div className="bg-white rounded-lg p-4 shadow-sm">
...
//                       <p className="text-xs text-gray-500">SQFT × Paint price per sqft × 2 (both sides)</p>
//                     </div>
//                   </div>
//                 </div>
//                 <div className="bg-gray-50 rounded-lg p-6 border">
...
//                   <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
//                     <div className="bg-white rounded-lg p-4 shadow-sm"><label className="block text-sm font-medium text-gray-700 mb-2">SQFT</label><p className="text-xl font-bold text-blue-600">{(formData.sqft || 0).toFixed(2)} sqft</p></div>
//                     <div className="bg-white rounded-lg p-4 shadow-sm"><label className="block text-sm font-medium text-gray-700 mb-2">Weight (kg)</label><p className="text-xl font-bold text-teal-600">{(formData.weight || 0).toFixed(2)} kg</p></div>
...
export default function StoreInventoryList() {
  ...
  const handleMMChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setFormData(prev => {
      ...
      // Calculate SQFT directly from MM
      const newSqft = calculateSqftFromMM(widthMM, lengthMM)
      updated.sqft = newSqft
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const calculatedSheetCost = currentPricePerKg * weightValue
      // Paint cost multiplied by 2 for both sides
      const calculatedPaintCost = currentPaintPrice * sqftValue * 2
      
      // Create item with today's costs
      const newStoreItem = {
        partNumber: formData.partNumber,
        partName: formData.partName,
        category: formData.category,
