...
export default function OutwardChallanPage() {
  ...
  const [selectedChallan, setSelectedChallan] = useState<OutwardChallan | null>(null);
  // const [includeRates, setIncludeRates] = useState<boolean | null>(null); // Commented out as it's not used
  ...
  */
  ...
  const handlePrintBillClick = (challan: OutwardChallan) => {
    setSelectedChallan(challan);
    setShowRateModal(true);
  };

  const handleRateSelection = (include: boolean) => {
    // setIncludeRates(include); // Commented out as it's not used
    setShowRateModal(false);
    if (selectedChallan) {
      handlePrintBillFormat(selectedChallan, include);
    }
    setSelectedChallan(null);
    // setIncludeRates(null); // Commented out as it's not used
  };
  ...
  // Print function for Bill Format with rate inclusion option
  const handlePrintBillFormat = (challan: OutwardChallan, includeRates: boolean) => {
    const printWindow = window.open('', '_blank');
    if (printWindow) {
      printWindow.document.write(`
        <!DOCTYPE html>
        <html>
          <head>
            <title>Outward Challan - Bill Format - ${challan.autoChallanNumber}</title>
            <style>
              @import url('https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,100..1000;1,9..40,100..1000&display=swap');
                  ...
                  <table class="data-table">
                    <thead>
                      <tr>
                        ...
                        <th class="qty-column">Qty</th>
                        <th class="rate-column">Rate</th>
                        <th class="rate-column">Rate/SQFT</th>
                        <th class="numeric-column">Amount</th>
                      </tr>
                    </thead>
                    <tbody>
                      ${challan.items?.length > 0 ? 
                        challan.items.map((item, index) => `
                          <tr>
                            ...
                            <td class="numeric-cell">${(item.sqft || 0).toFixed(2)}</td>
                            <td class="center-cell">${item.qty || 0}</td>
                            <td class="numeric-cell">${includeRates ? (item.rate || 0).toFixed(2) : '-'}</td>
                            <td class="numeric-cell">${includeRates ? (item.ratePerSqft || 0).toFixed(2) : '-'}</td>
                            <td class="numeric-cell">${includeRates ? (item.amount || 0).toFixed(2) : '-'}</td>
                          </tr>
                        `).join('') : 
                        `<tr>
                          <td colspan="9" class="center-cell" style="padding: 10px;">
                    ...
                    ${challan.items?.length > 0 ? `
                      <tfoot>
                        <tr class="total-row">
                          <td colspan="6" style="text-align: right; font-weight: bold; padding: 2px 4px !important;"></td>
                          <td colspan="2" class="total-amount-heading" style="text-align: right;">Total Amount:</td>
                          <td class="numeric-cell" style="font-weight: bold; font-size: 11px; padding: 2px 4px !important;">${includeRates ? (challan.totalAmount || 0).toFixed(2) : '-'}</td>
                        </tr>
                      </tfoot>
                    ` : ''}
                  </table>
  ...
  // Print function for Quantity Format - S.NO, DESCRIPTION, QTY, and TOTAL PIECES
  const handlePrintQuantityFormat = (challan: OutwardChallan) => {
    const printWindow = window.open('', '_blank');
    if (printWindow) {
      printWindow.document.write(`
        <!DOCTYPE html>
        <html>
          <head>
            <title>Outward Challan - Quantity Format - ${challan.autoChallanNumber}</title>
            <style>
              @import url('https://fonts.googleapis.com/css2?family=DM+Sans:ital,opsz,wght@0,9..40,100..1000;1,9..40,100..1000&display=swap');
              ...
              * {
              ...
              .sno-column { width: 40px; }
              .description-column { width: 400px; }
              .qty-column { width: 50px; }
              .center-cell {
                text-align: center;
                font-family: 'DM Sans', sans-serif !important;
                  ...
                  <table class="data-table">
                    ...
                    <tbody>
                      ${challan.items?.length > 0 ? 
                        ...
                        `).join('') : 
                        `<tr>
                          <td colspan="3" class="center-cell" style="padding: 10px;">
                            No items found
                          </td>
                        </tr>`
                      }
                    ...
                    ${challan.items?.length > 0 ? `
                      <tfoot>
                        <tr class="total-row">
                          <td colspan="2" style="text-align: right; font-weight: bold; padding: 2px 4px !important;">Total Pieces:</td>
                          <td class="center-cell" style="font-weight: bold; font-size: 11px; padding: 2px 4px !important;">${challan.totalPieces || 0}</td>
                        </tr>
                      </tfoot>
                    ` : ''}
  ...
  const handleDownloadPDF = (challan: OutwardChallan, format: 'bill' | 'quantity') => {
    if (format === 'bill') {
      handlePrintBillClick(challan);
    } else {
      handlePrintQuantityFormat(challan);
    }
  };

  // Rate Selection Modal Component with Cancel button
  const RateSelectionModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
        <div className="bg-white rounded-lg p-6 max-w-md w-full mx-4 shadow-xl relative">
          <h3 className="text-xl font-bold text-gray-800 mb-4 font-dm-sans">Include Rates in Bill?</h3>
          <p className="text-gray-600 mb-6 font-dm-sans">
            Would you like to include rate information in this bill?
          </p>
          <div className="flex justify-end gap-3">
            <button
              onClick={closeModal}
              className="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 transition-colors font-dm-sans"
            >
              Cancel
            </button>
            <button
              onClick={() => handleRateSelection(false)}
              className="px-4 py-2 bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300 transition-colors font-dm-sans"
            >
              No, Hide Rates
            </button>
            <button
              onClick={() => handleRateSelection(true)}
              className="px-4 py-2 bg-[#8B5E3C] text-white rounded-md hover:bg-[#7A4F32] transition-colors font-dm-sans"
            >
              Yes, Include Rates
            </button>
          </div>
      ...
      <div className="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-200 font-dm-sans tracking-wide">
        {filteredChallans.length === 0 ? (
          <div className="text-center py-8 text-gray-500">
            ...
            {challans.length === 0 && (
              <Link href="/" className="text-[#8B5E3C] hover:underline ml-1 font-dm-sans tracking-wide">
                Create one now
              </Link>
            )}
          </div>
                ...
                {filteredChallans.map((challan) => (
                  <tr key={challan._id} className="hover:bg-gray-50 font-dm-sans tracking-wide">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() => handlePrintBillClick(challan)}
                          className="text-green-600 hover:text-green-800 transition-colors p-1.5 rounded-md font-dm-sans tracking-wide bg-green-50 border border-green-200 flex items-center gap-1"
                          title="Print Bill Format"
                        >
                        ...
                        <button
                          onClick={() => handlePrintQuantityFormat(challan)}
                          className="text-orange-600 hover:text-orange-800 transition-colors p-1.5 rounded-md font-dm-sans tracking-wide bg-orange-50 border border-orange-200 flex items-center gap-1"
                          title="Print Quantity Format"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 17h2a2 2 0 002-2v-4a2 2 0 00-2-2H5a2 2 0 00-2 2v4a2 2 0 002 2h2m2 4h6a2 2 0 002-2v-4a2 2 0 00-2-2H9a2 2 0 00-2 2v4a2 2 0 002 2zm8-12V5a2 2 0 00-2-2H9a2 2 0 00-2 2v4h10z" />
                          </svg>
                        ...
                        <button
                          onClick={() => handleDownloadPDF(challan, 'bill')}
                          className="text-purple-600 hover:text-purple-800 transition-colors p-1.5 rounded-md font-dm-sans tracking-wide bg-purple-50 border border-purple-200"
                          title="Download Bill Format as PDF"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                          </svg>
                        ...
                        <button
                          onClick={() => handleDownloadPDF(challan, 'quantity')}
                          className="text-indigo-600 hover:text-indigo-800 transition-colors p-1.5 rounded-md font-dm-sans tracking-wide bg-indigo-50 border border-indigo-200"
                          title="Download Quantity Format as PDF"
                        >
                          <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                          </svg>
