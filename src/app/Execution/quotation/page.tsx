...
export default function QuotationsListPage() {
  ...
  if (isLoading) {
    return (
      <div className={`min-h-screen flex items-center justify-center bg-white text-gray-800 ${dmSans.className} font-sans`}>
        <div className="flex flex-col items-center space-y-4">
          <div className="animate-spin rounded-full h-12 w-12 border-4 border-black border-t-transparent shadow-lg"></div>
        </div>
      </div>
    );
  }
  ...
  return (
    <ProtectedRoute allowedUser="execution">
      <div className={`min-h-screen bg-white text-black ${dmSans.className} tracking-wide`}>
        ...
        <main className="max-w-6xl mx-auto px-3 py-5 lg:pl-8">
          {/* Header with search */}
          <div className="mb-5 flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 pt-3 sm:pt-0">
            <h1 className="text-xl sm:text-2xl pt-8 font-bold text-[#8B5E3C]">
              All Quotations
            </h1>
            ...
            <div className="relative w-full sm:w-72">
              ...
              <input
                type="text"
                placeholder="Search quotations..."
                className="w-full pl-8 pr-16 py-1.5 text-sm border rounded-lg focus:outline-none focus:ring-2 focus:ring-[#8B5E3C]"
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
              {searchTerm && (
              ...
              <button className="absolute right-1 top-1 px-2.5 py-1 text-xs bg-[#8B5E3C] text-white rounded-md hover:bg-[#6d4a2f] transition">
                Search
              </button>
            </div>
          </div>
          ...
          {isMobileView ? (
            ...
            <div className="space-y-3">
              {filteredQuotations.length > 0 ? (
                filteredQuotations.map((q) => {
                  ...
                  return (
                    <div key={q.quotationId} className="border rounded-lg p-3 shadow-sm">
                      <div className="flex justify-between items-start">
                        <div>
                          <h3 className="font-medium text-sm">{q.quotationId?.toUpperCase() || '-'}</h3>
                          <p className="text-gray-600 text-xs">{q.company || '-'}</p>
                        </div>
                      ...
                      {expandedQuotation === q.quotationId && (
                        <div className="mt-2 space-y-1.5 text-xs">
                          ...
                          <Link
                            ...
                            className="block mt-2 text-center text-blue-600 hover:underline font-medium text-xs"
                          >
                            View Full Details
                          </Link>
                        </div>
            ...
            // Desktop view - table with no scrollbar
            <div className="border shadow-sm bg-white rounded-lg overflow-hidden">
              <table className="w-full table-fixed divide-y divide-gray-200">
                <thead className="bg-gray-50">
                  <tr>
                    <th className="px-2 py-2 text-left text-[10px] font-medium text-[#8B5E3C] uppercase tracking-wider w-[14%]">
                      Ref No.
                    </th>
                    <th className="px-2 py-2 text-left text-[10px] font-medium text-[#8B5E3C] uppercase tracking-wider w-[16%]">
                      Company
                    </th>
                    <th className="px-2 py-2 text-left text-[10px] font-medium text-[#8B5E3C] uppercase tracking-wider w-[16%]">
                      Project
                    </th>
                    <th className="px-2 py-2 text-left text-[10px] font-medium text-[#8B5E3C] uppercase tracking-wider w-[12%]">
                      Date
                    </th>
                    <th className="px-2 py-2 text-left text-[10px] font-medium text-[#8B5E3C] uppercase tracking-wider w-[14%]">
                      Sales Person
                    ...
                    <th className="px-2 py-2 text-left text-[10px] font-medium text-[#8B5E3C] uppercase tracking-wider w-[10%]">
                      Action
                    </th>
                  </tr>
                </thead>
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredQuotations.length > 0 ? (
                    filteredQuotations.map((q) => {
                      ...
                      return (
                        <tr key={q.quotationId} className="hover:bg-gray-50">
                          ...
                          <td className="px-2 py-2.5 text-xs text-gray-500 truncate">
                            {q.company || '-'}
                          </td>
                          <td className="px-2 py-2.5 text-xs text-gray-500 truncate">
                            {q.projectName || '-'}
                    ...
                    <tr>
                      <td colSpan={7} className="px-3 py-5 text-center text-xs text-gray-500">
                        {searchTerm ? 'No matching quotations found' : 'No quotations found yet'}
                      </td>
                    </tr>
                  )}
