...
//     // Process parts with ALL fields including dimensions and gauge
//     const processedParts: ProcessedPart[] = body.parts.map((part: Part, index: number) => {
//       const completedQty = part.completedQty || 0
//       const qty = part.qty || 0
//       const remainingQty = qty - completedQty
...
//       // CRITICAL: Preserve all dimension fields
//       const blankWidthMM = part.blankWidthMM || part.blankWidth || 0
//       const blankLengthMM = part.blankLengthMM || part.blankLength || 0
//       const blankWidthInch = part.blankWidthInch || (blankWidthMM ? blankWidthMM / 25.4 : 0)
//       const blankLengthInch = part.blankLengthInch || (blankLengthMM ? blankLengthMM / 25.4 : 0)
//       const blankSizeSqft = part.blankSizeSqft || 0
...
//         blankWidthInch: blankWidthInch,
//         blankLengthInch: blankLengthInch,
//         blankSizeSqft: blankSizeSqft,
//         weight: weightPerPiece,
//         sheetCost: part.sheetCost,
//         gauge: part.gauge,
...
//     console.log('Creating work order with parts that have ALL dimensions:', 
//       processedParts.map((p: ProcessedPart) => ({ 
//         name: p.partName, 
//         blankWidthMM: p.blankWidthMM,
//         blankLengthMM: p.blankLengthMM,
//         blankWidthInch: p.blankWidthInch,
//         blankLengthInch: p.blankLengthInch,
//         gauge: p.gauge,
//         material: p.material,
//         weight: p.weight,
//         blankSizeSqft: p.blankSizeSqft,
...
//     // Process parts for update - INCLUDING ALL dimensions and gauge
//     const processedParts: ProcessedPart[] = updates.parts.map((part: Part, index: number) => {
//       const completedQty = part.completedQty || 0
//       const qty = part.qty || 0
//       const remainingQty = qty - completedQty
...
//     console.log('Updating work order with parts that have ALL dimensions:', 
//       processedParts.map((p: ProcessedPart) => ({ 
//         name: p.partName,
//         partNo: p.partNo,
//         blankWidthMM: p.blankWidthMM,
...
// Helper function to calculate paint cost per piece (fallback only)
function calculatePaintCostPerPiece(blankSizeSqft: number, todayPaintCost: number): number {
  return blankSizeSqft * todayPaintCost
}
...
export async function POST(req: Request) {
  try {
    ...
    const processedParts: ProcessedPart[] = body.parts.map((part: Part, index: number) => {
      ...
      // CRITICAL: Use paintCostPerPiece from the part (which comes from store item)
      // Only calculate fallback if not provided
      const todayPaintCost = part.todayPaintCost || 0
      const paintCostPerPiece = part.paintCostPerPiece || calculatePaintCostPerPiece(blankSizeSqft, todayPaintCost)
