...
export default function EditWorkOrderSalesPage() {
  ...
  // Calculate GST summary whenever PO table changes
  useEffect(() => {
    if (formData.purchaseOrderSection.poTable.length > 0) {
      const subtotal = formData.purchaseOrderSection.poTable.reduce(
        (sum, item) => sum + (item.totalAmountPKR - (item.gstAmount || 0)), 0
      )
      
      const totalGST = formData.purchaseOrderSection.poTable.reduce(
  ...
  }, [formData.purchaseOrderSection.poTable])
  ...
  // Specialized handlers for nested objects
  const handleCustomerInfoChange = (field: keyof CustomerInfo, value: string) => {
    // Capitalize each word if the field is salesPerson
    const processedValue = field === 'salesPerson' ? capitalizeEachWord(value) : value;
  ...
  const handleTermsChange = (field: keyof TermsAndConditions, value: string | boolean | number) => {
    setFormData(prev => ({
      ...prev,
      salesOrderSection: {
        ...prev.salesOrderSection,
  ...
  const updatePOItem = (index: number, field: keyof POItem, value: string | number | boolean) => {
    setFormData(prev => {
      ...
      // Recalculate GST and total if relevant fields change
      if (field === 'unitRatePKR' || field === 'quantity' || field === 'gstApplicable' || field === 'gstPercentage') {
        ...
        if (updatedItem.gstApplicable) {
          const gstPercentage = Number(updatedItem.gstPercentage || 0)
          const gstAmount = subtotal * (gstPercentage / 100)
          updatedItem.gstAmount = gstAmount
          updatedItem.totalAmountPKR = subtotal + gstAmount
        } else {
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          ...
          <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <InputField 
                ...
                type="number" 
                value={formData.salesOrderSection.termsAndConditions.gstPercentage || ""} 
                onChange={(v) => handleTermsChange("gstPercentage", Number(v))} 
                fontClass={dmSans.className}
              />
              <InputField 
                label="Authorized By" 
          ...
          <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
            ...
            {formData.purchaseOrderSection.poTable.map((item, index) => (
              <div key={index} className="grid grid-cols-1 md:grid-cols-6 gap-3 border p-3 rounded mb-3">
                <TextareaField 
                  label="Description" 
                  value={item.description || ""} 
                  onChange={(v) => updatePOItem(index, "description", v)} 
                ...
                <InputField 
                  ...
                  fontClass={dmSans.className}
                />
                <div className={`flex flex-col ${dmSans.className} tracking-wide`}>
                  <label className="block text-sm font-medium mb-1 tracking-wide">GST</label>
          ...
          {formData.purchaseOrderSection.gstSummary && (
            <div className="bg-gray-50 p-4 rounded-lg shadow-sm">
              <h2 className={`text-lg font-semibold text-[#8B5E3C] mb-3 border-b pb-2 ${dmSans.className} tracking-wide`}>
                Purchase Order - GST Summary
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <InputField 
                  label="Subtotal (PKR)" 
                  type="number" 
                  value={formData.purchaseOrderSection.gstSummary.subtotal || ""} 
                  readOnly 
                  fontClass={dmSans.className}
                />
                <InputField 
                  label="Total GST (PKR)" 
                  type="number" 
                  value={formData.purchaseOrderSection.gstSummary.totalGST || ""} 
                  readOnly 
                  fontClass={dmSans.className}
                />
                <InputField 
                  ...
                  type="number" 
                  value={formData.purchaseOrderSection.gstSummary.grandTotal || ""} 
                  readOnly 
                  fontClass={dmSans.className}
                />
              </div>
...
function InputField({ 
  ...
  return (
    <div className={`${fontClass} tracking-wide ${className}`}>
      <label className="block text-sm font-medium mb-1 tracking-wide">
        {label}
        {required && <span className="text-red-500"> *</span>}
      </label>
      <input
        ...
        readOnly={readOnly}
        disabled={disabled}
        className={`w-full border border-gray-300 rounded-md p-2 ${fontClass} tracking-wide ${readOnly || disabled ? 'bg-gray-100' : ''}`}
      />
    </div>
  )
}
