...
const sendEmailNotifications = async (formData: FormData) => {
  const workOrderNumber = formData.workOrderSection.workOrderNumber
  const emailSubject = `Work Order Created - ${workOrderNumber}`
  ...
  const emailPromises = EMAIL_ENDPOINTS.map(endpoint => 
    fetch(endpoint, {
      ...
      body: JSON.stringify({
        ...
        message: `
          A new work order has been created with the following details:
  ...
  try {
    ...
    if (successfulEmails > 0) {
      console.log(`Successfully sent ${successfulEmails} email notifications with subject: ${emailSubject}`)
    } else {
...
export default function AddWorkOrderSalesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success("Work order saved and notifications sent successfully!", { 
        duration: 4000, 
        position: 'top-center' 
      })
        ...
        <form onSubmit={handleSubmit} className="mt-6 space-y-6">
          ...
          <div className="bg-blue-50 p-4 rounded-lg shadow-sm border border-blue-200">
            ...
            <p className={`text-sm text-blue-600 mb-2 ${dmSans.className} tracking-wide`}>
              Upon successful submission, notifications will be sent to:
            </p>
            ...
            <p className={`text-sm text-blue-600 mt-2 ${dmSans.className} tracking-wide`}>
              <strong>Email Subject:</strong> Work Order Created - {formData.workOrderSection.workOrderNumber || '[Work Order Number]'}
            </p>
          </div>
...
// Helper Components (keep these unchanged)
function InputField({ 
  label, 
  type = "text", 
  value, 
