...
export default async function WorkOrderDetailPage({ params }: { params: { id: string } }) {
  ...
  _id == $id &&
  lower(salesOrderSection.customerInfo.salesPerson) == "Anas Nayyar"
][0]{
  _id,
  _createdAt,
  ...
  return (
    ...
    <div className={`min-h-screen bg-white text-gray-800 ${dmSans.variable} font-sans`}>
      ...
      <main className="max-w-6xl mx-auto px-4 py-10 space-y-8">
        ...
        <div className="flex justify-between items-start border-b pb-6">
  <div className="space-y-2">
    <h1 className={`text-3xl font-bold text-[#8B5E3C] ${dmSans.className} tracking-wide`}>
      Work Order: {workOrderSection?.workOrderNumber || 'N/A'}
    </h1>
    <div className="flex flex-wrap items-center gap-4">
      <div>
        <span className={`font-semibold text-[#8B5E3C] ${dmSans.className} tracking-wide`}>Created:</span>{' '}
        <span className={`${dmSans.className} tracking-wide`}>{formatDate(_createdAt)}</span>
      </div>
      {workOrderSection?.date && (
        <div>
          <span className={`font-semibold text-[#8B5E3C] ${dmSans.className} tracking-wide`}>Order Date:</span>{' '}
          <span className={`${dmSans.className} tracking-wide`}>{formatDate(workOrderSection.date)}</span>
        </div>
      )}
      {salesOrderSection?.orderDetails?.poNumber && (
        <div>
          <span className={`font-semibold text-[#8B5E3C] ${dmSans.className} tracking-wide`}>PO Number:</span>{' '}
          <span className={`${dmSans.className} tracking-wide`}>{salesOrderSection.orderDetails.poNumber}</span>
        </div>
      )}
    </div>
  </div>
