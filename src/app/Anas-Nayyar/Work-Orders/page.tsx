...
export default function WorkOrderList() {
  ...
  useEffect(() => {
    const fetchWorkOrders = async () => {
      try {
        // Modified query to only fetch Anas Nayyar's work orders
        const query = `*[
  ...
  "salesPerson": salesOrderSection.customerInfo.salesPerson
}
`
        ...
        <div className="flex flex-col gap-6 border-b pb-6">
          <div className="space-y-2">
            <h1 className={`text-2xl sm:text-3xl pt-8 font-bold text-[#8B5E3C] tracking-wide ${dmSans.className}`}>
              Anas Nayyar Work Orders
            </h1>
            <p className={`text-gray-600 tracking-wide ${dmSans.className}`}>
              List of all work orders for Anas Nayyar
            </p>
          </div>
            ...
            <div className="space-y-4">
              {filteredWorkOrders.length === 0 ? (
                <div className="text-center py-4 text-gray-500">
                  No work orders found for Anas Nayyar
                </div>
              ) : (
                    ...
                    filteredWorkOrders.map((order) => (
                      <tr key={order._id} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-700 space-x-3">
                          <Link
                            href={`/Anas-Nayyar/Work-Order/${order._id}`}
                            className="text-blue-600 hover:text-[#6d4a2f] font-medium"
                          >
                            View
                          </Link>
                          {/* <Link
